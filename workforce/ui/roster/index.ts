 ...
 * `host.call`, so the day the desktop grows a Workforce client there is exactly
 ...
 * application. A call therefore fails `unavailable` today and will succeed
 * unchanged when that client lands.
 *
 * The fallback is `recorded.ts`, and screens are told which they got so they can
 * say so. **A module that hid the difference is one somebody eventually acts
 * on** — a manager must be able to tell whether they are looking at their hotel.
 */
