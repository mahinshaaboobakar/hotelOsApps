...
/// </para>
/// </remarks>
public sealed class RoomCareFixture : IAsyncLifetime
{
    ...
    /// </remarks>
    private readonly InstallerConvention _convention = new(RoomCareDbContext.Schema, Run);
    ...
    private readonly string _password = $"rc{Guid.NewGuid():N}"[..24];
    ...
    private async Task PrepareAsync()
    {
        ...
        _database = await ScratchDatabase.CreateAsync(
            new ScratchDatabaseSpec(
                NamePrefix: "hotelos_roomcare_test",
        ...
        if (_database is null)
        {
            throw new InvalidOperationException(
                "Room Care's characterisation tests require PostgreSQL and could not reach it "
    ...
    public RoomCareDbContext Context() =>
        Context((_database ?? throw NotInitialised()).ConnectionFor(ApplicationRole));

    private static RoomCareDbContext Context(string connection) =>
        new(new DbContextOptionsBuilder<RoomCareDbContext>()
            .UseSnakeCaseNamingConvention()
            .UseNpgsql(
                connection,
                npgsql => npgsql.MigrationsHistoryTable("__migrations", RoomCareDbContext.Schema))
            .Options);
...
public sealed class RoomCareCollection : ICollectionFixture<RoomCareFixture>
{
    /// <summary>The collection every characterisation class joins.</summary>
    public const string Name = "roomcare";
}
