...
import { inlineNumber, inlineSelect, panel, radio, refuse, reorderSheet, saveLine, sentence } from "./controls";
...
export function rules(host: HostApi, body: HTMLElement, nav: Nav, data: SetupData): void {
  ...
  const leads = panel("Who decides a room's condition",
  ...
  const priority = panel("Priority ladder", rungs, reorder, sub("Unsold departure"),
  ...
  const supervisor = panel("The supervisor steps in",
