...
//
// # Adopted, not written here — GG's `539daf4`, copied verbatim
//
// A COPY of `guestops/ui/preview/parta/serve.mjs`, taken 2026-09-10 under the
// owner's three port rules. It is the second file this stream has copied from a
// sibling today, and the second labelled the same way: an application is
// installable on its own and may not reach into a neighbour's tree, so until
// **ADR 0154's released test-support package** exists the choice is a copy or a
// wrong dependency. When that package lands, this file and `preview/boot/`
// are deleted in favour of it — which is what the label is for.
//
// **Jobs was the collision GG describes below.** 8853 was bound by this stream,
// rooted at `HotelOsApps/jobs/ui`, and a workforce capture was answered with a
// Jobs page. Adopting their instrument rather than writing a third one is the
