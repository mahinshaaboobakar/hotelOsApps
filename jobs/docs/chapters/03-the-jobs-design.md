...
| **The Kernel's health probe cancels every attempt** | `PROBE_INTERVAL` (250 ms) is passed to `reqwest` as each request's `.timeout(...)`, so a check that takes longer than the polling interval is cancelled for the whole 15-second budget and the install is undone. Jobs' first database round-trip on a cold connection takes ~265 ms. **`SHELL-Q41`, Z's**; the reproduction is in the certificate. Not tuned around here: a lighter check would squeak under and still be a race | every install of Jobs |
| **An installed application cannot consume events** | `EventConsumerHost` requires `ServiceRegistered`, which only `AddKernelRegistration` provides — and an installed application must not self-register, since the Kernel writes its registry row. **`SHELL-Q41`, Z's** | the boot of any application that subscribes |
| **Jobs' five widgets — the review, done 2026-09-05, and three findings** | Z drew all five (`1e28fc0`, `SHELL-Q35`) and the owner approved the canvas on 2026-09-03; the register gates the builds on **this stream's review**, which had never been done. It is done now, and does not come back clean: **Raised Today** counts by *intent* (Fix · Prepare · Deliver · Check), a concept the walkthrough removed when it replaced the job `type` with the catalogue's category › item — there is nothing to count by; **By Priority** uses emergency · high · normal · low where this design rules `P1 · P2 · P3 · NOT_TRIAGED`, and translating quietly would put two vocabularies in one product; **Due Soon** splits *deadline from the guest flow* from *deadline from the service*, and Jobs stores one `due_at` with `priority_decided_by` recording FLOW as a **priority** source, not a deadline one. The Board and Blocked are computable exactly as drawn. **And `jobs-now` — built, signed, registered — is not one of the five**: it is this chapter's own widget, nearest to *The Board* without being it, so either it is The Board under another name or Jobs ships six. **Ruled 2026-09-05 — `JOBS-Q3`: Jobs ships six.** *The Board* and *Blocked* are built (`WidgetProjection`, two module reads, two bundles, three widgets declared); By Priority and Due Soon are amended to this design's vocabulary (`e39d39e`) and await the build; the redrawn *Raised Today* frame is with the owner | two widgets, and the Raised Today frame |
| **The multi-day job with a hand-over plan** | **`JOBS-Q2`**, registered 2026-09-05 from Room Care's walkthrough (KK), routed through the architect. The owner's words: *"weeks of work — staff one works Monday, next day another staff because staff one may go to another shift."* What this design already has: an assignment row per hand-over, a work session per person's stretch, `job.assign`, hold and resume, steps — a job passes between hands and each person's work is named. What it lacks: an **assignment plan** (day or shift → person, drawn from Workforce's postings and filled ahead by the supervisor), **steps that carry across hands**, and **progress a consumer can read by event** — Room Care's blocked lane reads it. Confirmed as Jobs' to build and never to be worked around in Room Care: a second multi-day assignment there is the duplicated-concept defect. **The opening item of the next design round, after this certification closes — not scope now** | nothing in the running drive |
| **A real token will be refused until the tuples exist** | Zero authorization tuples name the property, so a person's token gets `403` on everything — GG met it first. **`AUTHZ-Q26`**, waiting on II's registration subsystem; the desktop-drive rows carry that id when they run | driving the controls from the desktop, not the surface itself |
| **A seeded developer is assigned in Identity and unknown to the graph** | Measured 2026-09-16, after CC landed `job.read`'s property scope. The module's call now reaches `AuthorizeBatch` and is answered *no* — where before the registry refused the question itself (*"may be checked against job, but the object is a property"*), so **the scope fix is in effect at runtime**, not merely in the file. What remains is a tuple: `identity.user_properties` holds a row for `hh@kochi.test` at this property, and `openfga.tuple` holds **no** `property#member` for them, while the owner's general manager holds both `member` and `general_manager`. So a user created by `scripts/seed-user` can sign in, is assigned to a property, and is invisible to every authorization check — the row was written to the database and never announced, and the graph learns from events. Not this repository's to fix (`scripts/seed-user` is HosPilotOS'); reported | the live drive, for this stream's own bearer — the owner's GM is unaffected |
| **The hold-warning control: one box in the drawing, two controls in the build** | **`DECISION`**, with the owner. Settings mockup 02 composes *"when breached ▾ + [15] min"* as a single field with a chip inside it; the build draws two — a select and a number input side by side (`ui/screens/settings/tabs.ts:140,143`). This is the fourth kind of unpairable node and the only one no rule reaches: element type is settled by ARCH-Q20, fixture and state by §8, and this one differs in **cardinality** — one node against two. §10 names `.row2` (line 762) as the form two adjacent fields take, so the shape the build uses is one the standard defines and the drawing's composite is not; but whether *"warn N minutes after breach"* is one value or two is a composition question, and nothing in 64 rules it. **An input holding children is not a thing you make true by wanting the number** | one Part A row, measured as unpairable with its reason |
| ~~Master Data was asked over the wire~~ | **fixed 2026-09-06** (`288bd41`): `MasterDataPropertyDirectory` held a `MasterDataServiceClient`, and that call could never have succeeded — an application presents a certificate and carries no access token, and **an application is not a platform service** (ADR 0093 §PKG-Q8). No flow had reached it, so the code looked right while being unrunnable; Workforce met the identical refusal the same day (`64ab494`). The reads are now keyless entities over `masterdata.properties`, `departments` and `locations` through ADR 0092 §4's install grant, which `hotelos_app_jobs` already held. `IPropertyDirectory` unchanged | nothing — and it would have blocked every flow that names a place |
| ~~`property#jobs_manager` was declared and published by nobody~~ | **built 2026-09-06** (`6ce7a2c`). The manifest declared `user.jobs_manager_granted` and nothing emitted it, so the relation was unreachable by construction — §7's table has called this *"the GM's action"* since the design was written, and the action did not exist. `job.configure` now carries `grantJobsManager` and `revokeJobsManager`, published through the SDK's appender with **both ends in the body**: the envelope names the administrator who decided, and granting them instead *"would look correct in every test where the two coincide"* (the Kernel's own words), so the guard uses two distinct ids. Jobs still writes no tuple; `jobs_manager_grant` is the record of an action, kept because a screen must show the GM who they have granted | nothing — the first `job.configure` holder still comes from Identity's `SetGeneralManager` |
| **The install is stranded, and comes back by reinstall rather than by resume** | A Kernel restart on 2026-09-05 demoted `jobs` and `guestops` to `failed` with *"secret packages/jobs/database not found"*. The credential exists: the secret store is content-addressed, and `sha256` of `machine/packages/jobs/database` matches the file sealed at 21:31:44 — the install's own minute — while the property-scoped path the resume asks for matches nothing. `04a56d6` (CC) rules the property side correct and records that installs made before it **are reinstalled, not migrated**. Two further facts this stream established: `resume` starts only rows whose state is `Running`, so a demoted row is never retried and the console has no `start` — the stranding is one-way (`INSTALL-Q75(4)`); and **the reinstall source is a HEAD build, never the registry's `.hopkg`**, which was built at 16:20 and carries one widget where this design now ships three bundles | the whole application-level drive, until CC's lifecycle lands |
| ~~`AddPlatformAuthentication` has no endpoints to pass~~ | **closed 2026-09-05 by `SHELL-Q40`** (Z): `AddHotelOsApplication` and `UseApplicationListeners` adopted, both doors bind on the Kernel's ports and the eight capabilities map | nothing |
